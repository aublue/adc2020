...
namespace SecondOfDecemberTaskOne
{
    public class PasswordOption
    {
       ...
       // private static readonly Regex OccurrenceRegex = new Regex(@"([0-9]+)(?=-)");
        ...
        public PasswordOption(String PassLine)
        {
            ...
            PassCharCount = Password.Count(x => x.Equals(PassChar));
            PasswordValid = (MinOccurrence <= PassCharCount && PassCharCount <= MaxOccurrence);
        }
